...
output_folder=/mnt/extra/data/iot2023-8class-http/raw
...
i=1
size=200
find "$input_folder/5" -type f | parallel "mkdir -p $output_folder/$i && tcpdump -r {} -w $output_folder/$i/{/} -C $size -Z root && find \"$output_folder/$i\" -type f | grep -P \"(.pcap)[0-9]+\" | xargs -d\"\n\" rm"

# DoS HTTP Flood
i=2
size=800
find "$input_folder/18" -type f | parallel "mkdir -p $output_folder/$i && tcpdump -r {} -w $output_folder/$i/{/} -C $size -Z root && find \"$output_folder/$i\" -type f | grep -P \"(.pcap)[0-9]+\" | xargs -d\"\n\" rm"
